...
- if you are analyzing a protein present in the NDB database, you may use the option (i) BUT CHECK THAT the coordinate files with base-pair and base-pair step morphological parameters is present in the database, and that all basepairs are present in these lists. In many cases, the files are not present and there are sometimes missing basepairs, which will result in errors or absurd deformation energies. This happens particularly in the case of extremely distorted DNA oligomers, where it can be difficult to properly define the base reference frames.
- in general, we recommend using the Web3DNA webserver on the PDB file of interest, and providing the output (.out) to ThreaDNA, together with the PDB file. AGAIN, CHECK THAT ALL BASEPAIRS WERE PROPERLY ANALYZED AND ARE PRESENT IN THE LIST. 
